...
async fn root() -> impl IntoResponse {
    let (login_link, logout_link) = if env::var("AUTH_SERVICE_IP").is_ok() {
        ("/auth/".to_string(), "auth/logout".to_string())
    } else {
        ("http://localhost:8080".to_string(), "http://localhost:8080/logout".to_string())
    };
...
async fn protected(jar: CookieJar) -> impl IntoResponse {
    ...
    let url = format!("http://{}:8080/verify-token", auth_hostname);
